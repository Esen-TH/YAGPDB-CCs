{{$Suggestion_Channel:=606353929374138370}}
{{$Logging_Channel:=716041074363203665}}
{{$Implemented_Channel:=716041074363203665}}
{{$Approved_Channel:=716041074363203665}}
{{$Mod_Roles:=cslice 606353862957203456}}
{{$Cooldown:=0}}
{{$Upvote:="upvote:524907425531428864"}}
{{$Downvote:="downvote:524907425032175638"}}
{{$Neutral := "neutral:720118005366784091"}}
...
{{if reFind `(?i)\bsuggest(ion)?\b` .Cmd}}
	...
	{{if or .StrippedMsg .Message.Attachments}}
		...
		{{else}}
			...
			{{sendDM "Suggestion submitted successfully. If you want to delete your suggestion, do so with `" $Prefix "deletesuggestion " $ID "`."}}
			{{addReactions $Upvote}}
		{{end}}
	{{else}}
